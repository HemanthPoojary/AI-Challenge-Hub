...
export function CompletionScreen({ teamData }: { teamData: any }) {
  ...
  }, [teamData.startTime, teamData.totalTime]);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {teamId ? (
        <div className="w-full px-4 pb-12 pt-4 border-t border-slate-800/80 bg-slate-950/40">
          <p className="text-center text-gray-400 text-sm mb-4 max-w-xl mx-auto">
            Live standings below update in real time until every team has finished. Watch rows animate as ranks change.
          </p>
          <LiveLeaderboard currentTeamId={teamId} fullList />
        </div>
      ) : null}
    </div>
  );
